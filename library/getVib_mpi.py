...
def getJ(i,j,dt,idt,dtRange,idtRange,iextraRange,ndt,vid,gList,pos,v0,cell,sL,vSL,n,trans,ntrans,run_t,tol_t,tstep,run_nt,tol_nt,leakyV,extralet,v1,s,savePlot,plotData,directory,rdpi):
    ...
    for jdt in xrange(idt,ndt):
        ...
        relt = run_t-dtt
        v,fired = bproceed(cell, v0, sL, gList, RList, vSL, spikes, n, sel, trans, trans + relt , 0, tstep, '', -1.0, 0, True, True)
...
def getExtralets(i,idt,iextraRange,gList,dt,v0,cell,sL,vSL,spikes,tstep,n,trans,ntrans,run_nt,relt,relit,leakyV,s):
    ...
    RList = np.zeros((n,2))
    RList[i,:] = getGH(dt,gList[i])
    evtmp, _ = bproceed(cell, v0, sL, gList, RList, vSL, spikes, n, sel, trans, trans + relt, 0, tstep, '', -1.0, 0, True, True)
...
def getNib(argv):
   ...
   # E/I split 1-74 | 75-134
    ...
    #===========================================================
    vRange = np.array([-74,-70,-61,-58],dtype='double')
    ...
    cell.set_volt(v0)
    leakyV, leakyDendV = leaky(cell, v0, synList, RList, vecStimList, n, trans, run_t + trans, tstep, loc, pos, True, True)
