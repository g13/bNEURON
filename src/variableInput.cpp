...
int main(int argc, char **argv)
{
    ...
    string theme;
    bool testing;
    bool TempVariableFlag = false;
    bool UniformRateStepFlag = true;
    bool LocVariableFlag = false;
    int nr;
    ...
    cmd_line_options.add_options()
		...
		("srate", po::value<vector<double>>()->multitoken()->composing(), "starting poisson rate E&I in cmdline")
		("erate", po::value<vector<double>>()->multitoken()->composing(), "ending poisson rate E&I in cmdline")
		("nrate", po::value<vector<int>>(&nr), "number of poisson rates E&I")
        ("var,T",po::value<bool>(&TempVariableFlag),"using variable input")
        ("varL,L",po::value<bool>(&LocVariableFlag),"using variable input")
        ("varfile",po::value<string>(&variableInputFile)->default_value("variableInput"),"table of variable input");
	Generic.add_options()
		("sratef", po::value<vector<double>>()->multitoken()->composing(), "starting poisson rate E&I in config file")
		("eratef", po::value<vector<double>>()->multitoken()->composing(), "ending poisson rate E&I in config file")
	...
	vector<double> srf = vm["sratef"].as<vector<double>>();
	vector<double> erf = vm["eratef"].as<vector<double>>();
    vector<double> sr = vm["srate"].as<vector<double>>();
    vector<double> er = vm["erate"].as<vector<double>>();
    int ndiff = 2;
    if (varL) ndiff = neuroLib.nSyn;
	if (sr.size()!=ndiff) {
		assert(sr.size()==0);
        assert(er.size()==0);
		sr.assign(srf.begin(),srf.end());
		er.assign(erf.begin(),erf.end());
	} else {
        assert(er.size()==ndiff);
    }
    for (i=0; i<ndiff; n++) {
	    assert(sr[i]>=0.0 && er[i] >= 0.0);
    }
    if (!seed) seed = seed0;
    assert(nr>=1);
    if (nr == 1) {
        string prefix = "E"+to_string(static_cast<int>(sr[0]))+"-I"+to_string(static_cast<int>(sr[1]))+"-corrL"+to_string(static_cast<int>(neuroLib.tol_tb-ignore_t))+"-t"+to_string(static_cast<int>(run_t))+"-s"+to_string(static_cast<unsigned int>(seed));
    } else {
        string prefix = to_string(static_cast<int>(sr[0])) + "E"+to_string(static_cast<int>(er[0]))+ "-" + to_string(static_cast<int>(sr[1])) + "I"+to_string(static_cast<int>(er[1]))+"-corrL"+to_string(static_cast<int>(neuroLib.tol_tb-ignore_t))+"-t"+to_string(static_cast<int>(run_t))+"-s"+to_string(static_cast<unsigned int>(seed));
    }
    theme = "-" + theme;
