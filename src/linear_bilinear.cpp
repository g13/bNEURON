...
unsigned int bilinear_nSyn(Cell &cell, vector<vector<double>> &spikeTrain, double rd, vector<double> &v, nNL &neuroLib, nNS &neuron, double run_t, double ignore_t, vector<double> &tsp, double vCross, double vBack, int afterCrossBehavior, bool spikeShape, bool dtSquare, int itrial, bool sliceDebugPlot){
    ...
    if (neuron.tin.size() > 0) {
        ...
        for (i=0; i<neuron.tin.size(); i++) {
            ...
            if (lb::debug2) {
                if (i_b == i && i != 0) cout << "first input after cross " << v[vs] << "\n";
            }
            for (k=i; k>i_b; k--) {
                ...
                if (lb::debug2) {
                    cout << "    " << j << "th " << neuron.ei[neuron.inID[j]] << ", " << i <<"th " << neuron.ei[neuron.inID[i]] <<  "\n";
                    cout << "        b " << k << " v " << v[vs] << "\n";
                    cout << "        tl " << tl << " vs + tl " << vs + tl << "\n";
                    cout << "        dtTar  " << dtTarget << "\n";
                    cout << "        run_nt  " << run_nt << "\n";
                }
            ...
            for (k=vs;k<vc;k++) {
                ...
                while (v[k] > vCross) {
                    ...
                    ncross = ncross +1;
                    if (lb::debug) {
                        cout << "crossed at " << k*tstep << ", start ith " << ith << " t= " << neuron.tin[ith] << "\n";
                        cout << "v[vs] " << v[k]  << " vCross " << vCross << "\n";
                    }
                    if (spikeShape) {
                    ...
                    assert(ith>=ith_old);
                    if (lb::debug) {
                        cout << "backed at " << vs*tstep << ", end ith " << ith << " t= " << neuron.tin[ith] << "\n";
                        cout << "v[ve] " << v[vs] << " vCross " << vCross << " vBack " << vBack << "\n";
                    }
                    tCross_old = vs*tstep;
                    vCross_old = v[vs];
                    if (v[vs] > vBack) {
                        if (vs < run_lt) {
                            cout << " NEURON simulation has some problem" << "\n";
                            assert(vs == run_lt);
                        }
                    }
                    if (spiked){
                    ...
                    if (spikeShape) {
                        if (spiked) {
                            ...
                            cout << "vas ended with " << v[vs+tl-1] << " at " << vs+tl-1 << "\n";
                        } else {
                    ...
                    if (afterCrossBehavior>0) {
                        ...
                        if (lb::debug) {
                            cout << "   finished afterspike sbPSP adjustment" << "\n";
                        }
                        ...
                        if (lb::debug) {
                            cout << " vc " << vc << "\n";
                            cout << "v k at vc-1: " << v[k] << "\n";
                        }
                        for (j=vs; j<vc; j++) {
                            if (v[j] > vCross) {
                                k = j;
                                if (lb::debug) {
                                    cout << "need to cross again at " << k << "\n";
                                }
                                break;
                            }
                        }
                        ...
                        if (lb::debug) {
                            cout << " no readjust input, only leakage" << "\n";
                        }
                        if (ith<neuron.tin.size()-1) {
                            ve = static_cast<size>(neuron.tin[ith+1]/tstep);
                            i_b = ith + 1;
                ...
                if (crossed) {
                    i = ith;
                    if (lb::debug) {
                        cout << " last input during cross " << i << "\n";
                    }
                    break;
                }
            }
...
unsigned int bilinear0_nSyn(Cell &cell, vector<vector<double>> &spikeTrain, double rd, vector<double> &v, nNL &neuroLib, nNS &neuron, double run_t, double ignore_t, vector<double> &tsp, double vCross, double vBack, int afterCrossBehavior, bool spikeShape, bool dtSquare, int itrial, bool sliceDebugPlot){
    ...
    if (vinit!=neuron.vRest){
        interpVinit(v,0,neuroLib.vLeak,neuroLib.vRange,nv,vinit,tl);
        cout << " bilinear0 need to start at resting potential for best accuracy" << "\n";
    }
    double rv;
    size iv0, jv;
    getNear(neuroLib.vRange, neuroLib.nv, vTarget, rv, iv0, jv);
    cout << " vRest at " << iv0 << "th vRange" << "\n";
    if (neuron.tin.size() > 0) {
        vs = static_cast<size>(neuron.tin[0]/tstep);
        for (i=0; i<neuron.tin.size(); i++) {
            if (lb::debug2) {
