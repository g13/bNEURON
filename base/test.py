...
def test(vi,vRange,cell,vThres,gList,loc,pos,sL,vSL,n,tstep,run_t,alphaR,normVrest, spikeTrain, sel, sender):
    ...
    if normVrest:
        cell.set_volt(v0)
    V, fired, dendV = bproceed(cell, v0, vThres, sL, gList, vSL, spikeTrain, n, sel, run_t, tstep, fign, pos, loc, alphaR, normVrest)
    if fired:
        print 'fired'
    print 'test', vi,
    sender.send(np.array([vi, V, dendV],dtype='object'))
    print 'sent'

def plotTest(vRange,cell,vSL,sL,pos,loc,gList,vThres,n,tstep,run_t,alphaR,normVrest,spikeTrain,sel,fign,fmt,datafn):
    nv = vRange.size
    V = np.empty((nv,run_nt))
    dendV = np.empty((nv,n,run_nt))
    jobs = []
    receivers = []
    for vi in xrange(nv):
    ...
    ax1.plot(np.arange(run_nt)*tstep,V.T,lw=.5)
    ax1.set_ylim((-67,-55))
    ax2 = fig.add_subplot(1,2,2)
    dV = V[:,run_nt-1]-V[:,0]
    ax2.plot(vRange,dV,lw=.5)
    pyplot.savefig(fign+'.'+fmt,format=fmt,bbox_inches='tight',dpi=600)
    with open(datafn+'.npz','w') as datafile:
        np.savez(datafile, dV=dV, V=V, vRange=vRange)

if __name__ == '__main__':
    fmt = 'png'
    datafn0 = 'dV-nonNormVrest'
    g0 = 32.0*2e-4
    maxE = 5
    tstep = 1.0/10.0
    run_t = 1000.0
