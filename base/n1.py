...
def run(cell, v0, vBack, tref, vThres, synList, RList, n, trans, oneGo, t0, alphaR = True, loc=np.array([]), pos=np.array([]), pas = False, v=h.Vector, t=h.Vector(), cpi = False, rdv=[h.Vector()]):
    ...
    print "preTrans at ", h.t, ": ", synList[0].g, synList[0].h, cell.soma(0.5).v
    ...
    if len(loc)>0:
        for i in xrange(n): 
            ...
            if alphaR:
                ...
                print synList[i].R
    checkList = [];
    print "initial at ", h.t, ": ", synList[0].g, synList[0].h, cell.soma(0.5).v
    if not pas:
        if oneGo:
            while int(round(h.t/h.dt)) < int(round(h.tstop/h.dt)):
                h.fadvance()
                print "tracking at ", h.t, ": ", synList[0].g, synList[0].h, cell.soma(0.5).v
...
if __name__ == '__main__':
    ...
    #v_pre = -60
    #for v_pre in vRange:
    v, fired, tsp, ntrans = proceed(cell, v_pre, synList, RList0, vecStimList, vecTuple, n, trans, trans+run_t, vBack, tref, vThres, oneGo, t0, tstep, loc, pos, dendVclamp, alphaR, getDendV, monitorDend, pas, True, 'slice1-'+str(-v_pre), False, False)
    cell.set_volt(-70.0)
    v1, fired, tsp, ntrans = proceed(cell, v_pre, synList, RList, vecStimList, vecTuple0, n, trans, trans+run_t, vBack, tref, vThres, oneGo, t0, tstep, loc, pos, dendVclamp, alphaR, getDendV, monitorDend, pas, True, 'slice2-'+str(-v_pre), False, True)
